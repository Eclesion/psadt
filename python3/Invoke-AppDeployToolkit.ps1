...
$Global:Pkg = @{
    ...
    Lang         = 'en-US'
    Revision     = '01'
    InstallTitle = 'Python 3.14 (x64) en-US with scientific packages'
    ...
    # Arguments
    InstallArgs   = '/quiet InstallAllUsers=1 PrependPath=1 Include_pip=1 SimpleInstall=1 CompileAll=1'
    UninstallType = 'EXE'
    UninstallFile = 'python-3.14.3-amd64.exe'
    ...
    PostInstallScript   = 'PythonPackages.ps1'
    PreUninstallScript  = $null
    PostUninstallScript = $null

    # Python package installation options (used in PostInstallScript)
    PythonExePath = "$env:ProgramFiles\Python314\python.exe"
    PipPackages = @(
        'pandas',
        'numpy',
        'matplotlib',
    ...
    DetectionMode = 'Exe'      # Uninstall | Exe | RegistryKey | RegistryValue | Custom

    # Uninstall detection
    DetectDisplayNameRegex = $null
    DetectMinVersion       = $null

    # EXE detection
    DetectExePaths = @(
        @{
            Path       = "$env:ProgramFiles\Python314\python.exe"
            MinVersion = '3.14.3'
        }
    )
    ...
    ProcessesToClose = @('python', 'python3', 'python3.14', 'py', 'pythonw', 'pip')
