...
$Global:Pkg = @{
    ...
    Lang         = 'en-US'
    Revision     = '01'
    InstallTitle = 'GIMP 3.2.0 (x64) en-US'

    # Installer type: EXE | MSI | MSIX | WINGET | SCRIPT
    InstallerType = 'EXE'

    # Files (relative to .\Files)
    InstallerFile   = 'gimp-3.2.0-setup.exe'
    TransformFile   = $null
    PatchFiles      = @()
    MsiProductCode  = $null

    # Arguments
    InstallArgs   = '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /TYPE="FULL" /ALLUSERS /NOCANCEL /NOCLOSEAPPLICATIONS'
    ...
    # Uninstall detection
    DetectDisplayNameRegex = '^(?i)GIMP(?:\s+\d+(?:\.\d+)+)?$'
    DetectMinVersion       = '3.2.0'

    # EXE detection
    DetectExePaths = @(
        @{
            Path       = "${env:ProgramFiles}\GIMP 2\bin\gimp-2.10.exe"
            MinVersion = '3.2.0'
        }
    )
